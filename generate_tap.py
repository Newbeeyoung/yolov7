...
def train(hyp, opt, device, tb_writer=None):
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            #         loss *= 4.
            with amp.autocast(enabled=cuda):
                delta = defender.perturb(model, compute_loss_ota, imgs, targets.to(device))
                # pdb.set_trace()
            
                def_noise[i*batch_size:(i+1)*batch_size] = delta.cpu().numpy()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_name', type=str, default='TAP',
                        help='tap save name')
