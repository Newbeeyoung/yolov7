...
from utils.datasets import create_dataloader,create_ue_dataloader
...
def train(hyp, opt, device, tb_writer=None):
    ...
    # Trainloader
    dataloader, dataset = create_ue_dataloader(train_path, imgsz, batch_size, gs, opt,
    ...
    def_noise = np.zeros([len(dataset), 3, 640, 640], dtype=np.float16)
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            with amp.autocast(enabled=cuda):
                def_x = defender.perturb(model, compute_loss_ota, imgs, targets.to(device))
                
                def_noise[i*batch_size:(i+1)*batch_size] = (def_x-imgs).cpu().numpy()

                model.train()
                pred = model(def_x)  # forward
    ...
    with open(os.path.join(save_dir, '{}-def-noise.pkl'.format(opt.save_name)), 'wb') as f:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_name', type=str, default='EM',
                        help='tap save name')
